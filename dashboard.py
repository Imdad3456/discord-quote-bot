...
.stations{display:grid;grid-template-columns:repeat(4,1fr);gap:10px}.station{min-height:115px;text-align:left;padding:15px;position:relative;overflow:hidden}.station b{display:block;font-size:16px;margin-top:27px;text-transform:capitalize}.station span{font-size:24px}.station.active{border-color:#a78bfa;background:#2a2040}.station:after{content:'▶';position:absolute;right:13px;top:13px;opacity:.55}.station-search{width:min(320px,100%);margin-bottom:14px}
...
@media(max-width:800px){.layout{grid-template-columns:1fr}.playing{grid-template-columns:105px 1fr}.art{width:105px}.song{font-size:20px}.stations{grid-template-columns:repeat(2,1fr)}.search{flex-wrap:wrap}.search input{width:100%;flex-basis:100%}.volume{margin-left:0}header{padding:0 18px}}@media(max-width:430px){.stations{grid-template-columns:1fr}}
...
<script>
const token=location.pathname.split('/').filter(Boolean).pop(),icons={'chill jazz':'☕','pop':'✨','2015 hits':'📻','hip hop':'🎤','r&b':'🌙','rock classics':'🎸','indie':'🌿','electronic':'⚡','afrobeats':'🌍','latin':'💃','country':'🤠','reggae':'🌴','lo-fi':'📚','80s hits':'🕹️','90s hits':'💿','2000s throwbacks':'📱','workout':'💪','party':'🎉'};
...
function filterStations(g,v){document.querySelectorAll(`#stations${g} .station`).forEach(x=>x.hidden=!x.dataset.name.includes(v.toLowerCase()))}function station(s,p){return `<button data-name="${esc(s)}" class="station ${p.radio===s?'active':''}" onclick="radio('${p.guild_id}','${s}')"><span>${icons[s]||'♫'}</span><b>${esc(s)}</b><small class=muted>${p.radio===s?'On air':'Start station'}</small></button>`}
function card(p){let c=p.current,percent=c&&c.length?Math.min(100,p.position/c.length*100):0,cover=c&&c.artwork?esc(c.artwork):'data:image/svg+xml,%3Csvg xmlns=%22http://www.w3.org/2000/svg%22 width=%22170%22 height=%22170%22%3E%3Cdefs%3E%3ClinearGradient id=%22g%22%3E%3Cstop stop-color=%22%236d3bb5%22/%3E%3Cstop offset=%221%22 stop-color=%22%23151822%22/%3E%3C/linearGradient%3E%3C/defs%3E%3Crect width=%22170%22 height=%22170%22 rx=%2218%22 fill=%22url(%23g)%22/%3E%3Ctext x=%2285%22 y=%22108%22 text-anchor=%22middle%22 font-size=%2264%22 fill=%22white%22%3E%E2%99%AB%3C/text%3E%3C/svg%3E',connect=p.connected?'':`<div class=connect><span class=muted>Play in</span><select id="c${p.guild_id}">${p.channels.map(x=>`<option value="${x.id}">${esc(x.name)}</option>`).join('')}</select></div>`;return `<section class=server><div class=server-head><div><h1>${esc(p.guild)}</h1><div class=sub>${p.connected?'Connected to voice':'Choose a voice channel to start'}</div></div>${p.radio?`<span class=radio-state>● ${esc(p.radio)} radio</span>`:''}</div>${connect}<div class="panel wide" style="margin-top:16px"><div class=label>Search & play</div><div class=search><input id="q${p.guild_id}" placeholder="Search a song or paste a YouTube / Spotify link" onkeydown="if(event.key==='Enter')add('${p.guild_id}')"><button class=primary onclick="add('${p.guild_id}')">Search & add</button></div><div class=status id="s${p.guild_id}"></div></div><div class=layout><div class="panel wide"><div class=label>Now playing</div><div class=playing><img class=art src="${cover}" alt="Album artwork"><div><div class=song>${c?esc(c.title):'Nothing playing'}</div><div class=artist>${c?esc(c.author):'Pick a song or station to begin'}</div><div class=progress><i style="width:${percent}%"></i></div><div class=times><span>${time(p.position)}</span><span>${c?time(c.length):'0:00'}</span></div><div class=controls><button onclick="act('${p.guild_id}','shuffle')">⇄</button><button class=round onclick="act('${p.guild_id}','pause')">Ⅱ</button><button class="round play" onclick="act('${p.guild_id}','resume')">▶</button><button class=round onclick="act('${p.guild_id}','skip')">▶|</button><button class=danger onclick="act('${p.guild_id}','stop')">Stop</button><div class=volume>🔊 <input id="v${p.guild_id}" type=number min=0 max=100 value="${p.volume}"><button onclick="act('${p.guild_id}','volume',document.querySelector('#v${p.guild_id}').value)">Set</button></div></div></div></div></div><div class="panel wide"><div class=queue-head><div><div class=label>Explore radio</div><b>${p.stations.length} stations</b></div>${p.radio?`<button onclick="act('${p.guild_id}','radio_off')">Turn off</button>`:''}</div><input class=station-search placeholder="Filter by genre, decade, or mood" oninput="filterStations('${p.guild_id}',this.value)"><div class=stations id="stations${p.guild_id}">${p.stations.map(s=>station(s,p)).join('')}</div></div><div class="panel wide"><div class=queue-head><div><div class=label>Up next</div><b>${p.queue.length} song${p.queue.length===1?'':'s'} queued</b></div><button onclick="act('${p.guild_id}','clear')">Clear queue</button></div><div class=queue>${p.queue.length?p.queue.slice(0,25).map((t,i)=>`<div class=track><span class=num>${i+1}</span><div><b>${esc(t.title)}</b><small>${esc(t.author)}</small></div><small>${time(t.length)}</small></div>`).join(''):'<div class=empty>Your queue is empty.</div>'}</div></div></div></section>`}
